...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.vblock = VisionTransformerBlock(
            dim=int(embed_dim * 2 ** 0), num_heads=3, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, 
            qk_scale=None, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=0., norm_layer=norm_layer
        )
    ...
    def get_first_selfattention(self, x):
        ...
        if self.absolute_pos_embed is not None:
            x = x + self.absolute_pos_embed
        x = self.pos_drop(x)
        return self.vblock(x, return_attention=True)
...
def swin_tiny_patch4_window7_224(embed_layer, num_classes=0, pretrained=False, **kwargs):
    model = SwinTransformer(embed_layer=embed_layer, num_classes=num_classes,
