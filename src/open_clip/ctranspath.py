...
class SwinTransformer(nn.Module):
    ...
    def lock(self, unlocked_groups=0, freeze_bn_stats=False):
       ...
       # If some groups should remain unlocked (i.e., trainable), set them here
        if unlocked_groups != 0:
            # Define groups of parameters that can be unlocked together
            groups = [
                [self.patch_embed, self.absolute_pos_embed],
                self.layers[:-1],
                [self.layers[-1], self.norm],
                self.head,
            ]
...
# This is the model that we use for our approach
